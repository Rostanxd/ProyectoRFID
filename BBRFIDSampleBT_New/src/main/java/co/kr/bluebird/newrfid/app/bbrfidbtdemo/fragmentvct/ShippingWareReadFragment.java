 ...
 */
public class ShippingWareReadFragment extends Fragment {


    public ShippingWareReadFragment() {
    ...
    private static final String TAG = ShippingWareReadFragment.class.getSimpleName();
    ...
    public static ShippingWareReadFragment newInstance() {
        return new ShippingWareReadFragment();
    }
...
//            mReader.RF_SetRssiTrackingState(SDConsts.RFRssi.ON);
        ...
        public void onClick(View v) {
            ...
            switch (id) {
                ...
                case R.id.next_imgbtn:
                    if (mShippingWareGenerateFragment == null)
                        mShippingWareGenerateFragment = mShippingWareGenerateFragment.newInstance();
                    ...
                    if(epcsLeidos.size() > 0) {
                        ...
                        mShippingWareGenerateFragment.setArguments(args);

                        FragmentTransaction ft = getFragmentManager().beginTransaction();
                        ft.replace(R.id.content, mShippingWareGenerateFragment);
    ...
    private static class UpdateStopwatchHandler extends Handler {
        private final WeakReference<ShippingWareReadFragment> mExecutor;
        public UpdateStopwatchHandler(ShippingWareReadFragment f) {
            mExecutor = new WeakReference<>(f);
        }

        @Override
        public void handleMessage(Message msg) {
            ShippingWareReadFragment executor = mExecutor.get();
            if (executor != null) {
    ...
    private static class WebServiceHandler extends Handler {
        private final WeakReference<ShippingWareReadFragment> mExecutor;
        public WebServiceHandler(ShippingWareReadFragment f) {
            mExecutor = new WeakReference<>(f);
        }
    ...
    private static class InventoryHandler extends Handler {
        private final WeakReference<ShippingWareReadFragment> mExecutor;
        public InventoryHandler(ShippingWareReadFragment f) {
            mExecutor = new WeakReference<>(f);
        }
